...
path_name = f"veres_easy_{args.name}"
path_name_growth = f"veres_easy_{args.name}_growth"

dataset = VeresDataset()
multipliermodel = MultiplierModel(data_dim=30)
state_dict = torch.load("checkpoints/veres_easy_0_best_multiplyer_model.pt")
...
visualizeresult(dataset, results, path_name, "umap", 
                highlight_times=[0.00, 1.00, 2.00, 3.00, 4.00, 5.00, 6.00, 7.00], visualization_batch=750)
