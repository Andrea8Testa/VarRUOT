...
from dataset import SimulationDataset, MouseDataset, VeresDataset, ZebrafishDataset
...
path_name = f"zebra_{args.name}"
path_name_growth = f"zebra_{args.name}_growth"

dataset = ZebrafishDataset()
multipliermodel = MultiplierModel(data_dim=50)
state_dict = torch.load("checkpoints/zebra_0_best_multiplyer_model.pt")
...
visualizeresult(dataset, results, path_name, "umap", 
                highlight_times=[0.00, 1.00, 2.00, 3.00, 4.00, 5.00, 6.00], visualization_batch=750)
