...
class VarRUOT_modified():
    ...
    def calculate_smoothed_g(self, m, growth_coeff):

        def safe_fractional_power(x, exponent):
        ...
        g_lower = safe_fractional_power((2 * growth_coeff) / (15 * m_lower), 15/13)
        g_upper = safe_fractional_power((2 * growth_coeff) / (15 * m_upper), 15/13)
        

        g_base = safe_fractional_power((2 * growth_coeff) / (15 * m), 15/13)
        
        g_linear = g_lower + ((m - m_lower) / (m_upper - m_lower)) * (g_upper - g_lower)
        

        mask = (m >= m_lower) & (m <= m_upper)
        
        return torch.where(mask, g_linear, g_base)
    ...
    def compute_particle_dynamics(self, x0, m0, t_samples):
        ...
        class ParticleSDE(torchsde.SDEIto):
            ...
            def f(self, t, y):
                ...
                g_val = self.parent.calculate_smoothed_g(self.parent.multiplyer_model(t, x), self.parent.cfg.growth_coeff)
                if g_val.ndim > 1:
    ...
    def compute_HJB_growth_term(self, t, x):
        ...
        frac1_top = 2 * self.cfg.growth_coeff
        frac1_bottom = 15 * phi
        frac1 = (frac1_top / (torch.abs(frac1_bottom) + 1e-4))**2
        frac1 = frac1 ** (1/13)


        result = frac1 * self.cfg.growth_coeff * (-13/15)
        return result
    ...
    def compute_total_HJB(self, t, x, m):
        ...
        # print(output**2)
        output = torch.sum(output**2 * m.unsqueeze(1)) / torch.sum(m)
        # print(output)
        return output
    ...
    def compute_matching_loss(self, ):
        ...
        for i in range(1, len(sorted_time_points)):
            ...
            tot_ot_loss += ot_loss.item()
            x_current = x_sim
            m_current = m_sim 
        return total_loss, tot_ot_loss
    ...
    def compute_action_loss(self):
        ...
        for i in range(len(t_samples)):
            ...
            this_g = self.calculate_smoothed_g(self.multiplyer_model(this_t, this_x), self.cfg.growth_coeff)
            if this_g.ndim > 1:
                this_g = this_g.squeeze(-1)
            loss_g = self.cfg.growth_coeff * torch.mean((safe_fractional_power(this_g, 2/15)) * this_m)
    ...
    def visualize2dresult(self, save_path=None, highlight_times=None):
        ...
        def dynamics(t, state):
            ...
            g_val = self.calculate_smoothed_g(self.multiplyer_model(t, x), self.cfg.growth_coeff)
            if g_val.ndim > 1:
                g_val = g_val.squeeze(-1)
            dm_dt = g_val * m
            return dx_dt, dm_dt
        ...
        if orig_dim > 2:
            ...
            pca.fit(sample_np)
            x_traj_flat_2d = pca.transform(x_traj_flat)
        ...
        for i in subsample_indices:
            ...
            points = x_traj_2d[:, i, :].reshape(-1, 1, 2)
        ...
        for particle in all_particles:
            ...
            if particle_np.shape[-1] > 2 and pca is not None:
                particle_2d = pca.transform(particle_np)
            else:
                particle_2d = particle_np
            ax.scatter(particle_2d[:, 0], particle_2d[:, 1],
        ...
        if highlight_times is not None and len(highlight_times) > 0:
            ...
            for idx, t_high in enumerate(highlight_times):

                closest_idx = np.argmin(np.abs(t_samples_np - t_high))
                ax.scatter(
                    x_traj_2d[closest_idx, :, 0],
                    x_traj_2d[closest_idx, :, 1],
                    color=colors[idx],
                    s=30,
                    marker='o',
                    label=f"t = {highlight_times[idx]:.2f}",
                    alpha=0.6
                )
    ...
    def visualize2dprob(self, grid_range=(-1.5, 1.5), num_points=50, t=0):

        x_vals = np.linspace(grid_range[0], grid_range[1], num_points)
        y_vals = np.linspace(grid_range[0], grid_range[1], num_points)
        XX, YY = np.meshgrid(x_vals, y_vals)
        grid_points_2d = np.stack([XX.ravel(), YY.ravel()], axis=1)  # shape: [num_points*num_points, 2]
        ...
        with torch.no_grad():
            ...
            probs = self.log_prob_model(t_tensor, grid_tensor)

        probs = torch.exp(probs)
        probs = probs.view(XX.shape).cpu().numpy()

        plt.figure(figsize=(6, 5))
        cp = plt.contourf(XX, YY, probs, levels=50, cmap='jet')
        plt.contour(XX, YY, probs, levels=15, colors='k', linewidths=0.5)
        if orig_dim > 2:
            plt.title("Prob (PCA Projection)")
            plt.xlabel("PC1")
            plt.ylabel("PC2")
        else:
            plt.title("Prob")
    ...
    def visualize2dgrowth(self, grid_range=(-1.5, 1.5), num_points=50, t=0):
        ...
        with torch.no_grad():
            t_tensor = torch.tensor(t, dtype=grid_tensor.dtype, device=self.cfg.device)
            growth = self.calculate_smoothed_g(self.multiplyer_model(t_tensor, grid_tensor), self.cfg.growth_coeff)
    ...
    def visualizedatagrowth(self, save_path = None):
        ...
        for t in all_time:
            ...
            this_growth = self.calculate_smoothed_g(self.multiplyer_model(t_tensor, particles), self.cfg.growth_coeff)
            particle_list.append(particles)
            growth_list.append(this_growth)


        ps = torch.cat(particle_list, dim=0).detach().cpu().numpy()  # shape: [N, orig_dim]
        gs = torch.cat(growth_list, dim=0).detach().cpu().numpy()      # shape: [N, ...]
        ...
        if orig_dim > 2:
            ...
            # title = "Data Growth (PCA Projection)"
        else:
            ps_2d = ps
            xlabel, ylabel = "x", "y"
            # title = "Data Growth"
        ...
        plt.scatter(xs, ys, c=gs, cmap='viridis', s=30, alpha=0.3)
        # plt.title(title)
        # plt.xlabel(xlabel)
        # plt.ylabel(ylabel)
        plt.xticks([])
        ...
        if save_path is None:
            ...
            os.makedirs("figs", exist_ok=True)
            path = os.path.join("figs", save_path + f".jpg")
            plt.savefig(path, dpi=300, bbox_inches='tight', pad_inches=0)
    ...
    def visualize_gaussian_density(self, sample_range=(-3, 3), num_points=100, sigma=0.2):
        ...
        if t0 is None or t1 is None:
            print("Warning: t=0 or t=1 not found in the data. Using the first two time points.")
            t0 = time_points[0]
