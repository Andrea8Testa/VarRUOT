...
from dataset import SimulationDataset, MouseDataset, VeresDataset, ZebrafishDataset, FFHQDataset
...
path_name = f"ffhq_{args.name}"
path_name_growth = f"ffhq_{args.name}_growth"

dataset = FFHQDataset()
multipliermodel = MultiplierModel(data_dim=512)
# state_dict = torch.load("checkpoints/ffhq_0_best_multiplyer_model.pt")
...
visualizeresult(dataset, results, path_name, "umap", 
                highlight_times=[0.00, 1.00], visualization_batch=750)
