...
from dataset import SimulationDataset, MouseDataset, VeresDataset, EmtDataset
...
path_name = f"emt_{args.name}"
path_name_growth = f"emt_{args.name}_growth"

dataset = EmtDataset()
multipliermodel = MultiplierModel(data_dim=10)
state_dict = torch.load("checkpoints/emt_0_best_multiplyer_model.pt")
...
visualizeresult(dataset, results, path_name, "raw", 
                highlight_times=[0.00, 1.00, 2.00, 3.00], visualization_batch=400)
