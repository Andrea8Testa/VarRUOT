...
path_name = f"mouse_n_{args.name}"
path_name_growth = f"mouse_{args.name}_growth"

dataset = MouseDataset()
multipliermodel = MultiplierModel(data_dim=2)
state_dict = torch.load("checkpoints/mouse_n_0_best_multiplyer_model.pt")
...
visualizeresult(dataset, results, path_name, "raw", 
                highlight_times=[0.00, 1.00, 2.00], visualization_batch=500)
